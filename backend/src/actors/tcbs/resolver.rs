...
use crate::algorithm::{Variables, Delegate, Format};
...
fn resolve_watching_tcbs_bid_ask_flow(actor: Arc<Addr<TcbsActor>>, resolver: &mut CronResolver) {
    resolver.resolve("tcbs.watch_bid_ask_flow".to_string(), move |task, _, _| {
        ...
        async move {
            let datapoints = actor.send(GetOrderCommand{ 
                    page: 0,
                })
                .await
                .unwrap();
           let fuzzy = task.pyfuzzy();
            ...
            let rule = Delegate::new()
                ...
                .map_err(|e| TcbsError{ message: e.to_string() })
                .unwrap();
            ...
            for response in datapoints {
                for order in response.data {
                    ...
                    for label in &labels {
                        match label.as_str() {
                            "p"   => { inputs.insert(label.clone(), order.p); },
                            "v"   => { inputs.insert(label.clone(), order.v as f64); },
                            "cp"  => { inputs.insert(label.clone(), order.cp); },
                            "rcp" => { inputs.insert(label.clone(), order.rcp); },
                            "ba"  => { inputs.insert(label.clone(), order.ba); },
                            "sa"  => { inputs.insert(label.clone(), order.sa); },
                            "hl"  => { inputs.insert(label.clone(), if order.hl { 1.0 } else { 0.0 }); },
                            "pcp" => { inputs.insert(label.clone(), order.pcp); },
                            _ => {},
                        };
                    }
                }
