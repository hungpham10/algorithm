...
impl Markov {
    ...
    fn calculate_proposal_std_dev(&self, data: &[f64], mu_current: f64, sigma_current: f64) -> f64 {
...
mod tests {
    ...
    fn test_metropolis_hastings_basic() {
        ...
        assert!(
            ...
            "Mu mean {} not close to true mu {}",
            mu_mean,
            true_mu
        );
        assert!(
            (sigma_mean - true_sigma).abs() < 2.0 * sigma_se,
            "Sigma mean {} not close to true sigma {}",
            sigma_mean,
            true_sigma
        );
    ...
    fn test_calculate_proposal_std_dev() {
        ...
        let markov = Markov::new(1000, Markov::log_prior, Markov::log_normal_distribute);
        ...
        assert!(
            ...
            "Proposal std_dev {} outside bounds [0.01, 10.0]",
            std_dev
        );
        ...
        assert!(
            std_dev > 0.0,
            "Proposal std_dev {} is not positive",
            std_dev
        );
    }
    ...
    fn test_metropolis_hastings_normal_distribution() {
        ...
        assert!(
            ...
            sigma_mean,
            sample_std,
            sigma_tolerance.max(0.1)
        );

        // Check that some samples were accepted
        assert!(
            !mu_samples.is_empty(),
            "No samples were accepted after burn-in"
        );
    }
    ...
    fn test_metropolis_hastings_histogram_exploration() {
        ...
        let (sample_bin_edges, sample_bin_counts) =
            create_histogram(&mu_samples, num_bins, min, max);
        ...
        assert!(
            density_diff < 0.05,
            "Histogram density difference {} too large",
            density_diff
        );
