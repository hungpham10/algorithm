pub fn histogram_with_condition<T>(
    data: &[(f64, T)],
    n_bins: usize,
    condition: fn(&(f64, T)) -> bool,
) -> Vec<(f64, f64, usize)> {
    ...
    for (i, item) in ret.iter_mut().enumerate().take(n_bins) {
        *item = (
            bellow + i as f64 * width,
            bellow + (i + 1) as f64 * width,
            0,
        );
    }
...
pub fn histogram_without_condition(data: &[f64], n_bins: usize) -> Vec<(f64, f64, usize)> {
