...
impl http::HttpModule for Module {
    fn module() -> &'static ngx_module_t {
        unsafe { &*::core::ptr::addr_of!(ngx_http_waf_module) }
    }
...
static mut NGX_HTTP_WAF_COMMANDS: [ngx_command_t; 2] = [
    ngx_command_t {
        name: ngx_string!("waf"),
        type_: (NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1) as ngx_uint_t,
        set: Some(ngx_http_waf_commands_set_enable),
...
ngx::ngx_modules!(ngx_http_waf_module);

#[used]
#[allow(non_upper_case_globals)]
#[cfg_attr(not(feature = "export-modules"), unsafe(no_mangle))]
pub static mut ngx_http_waf_module: ngx_module_t = ngx_module_t {
    ctx: std::ptr::addr_of!(NGX_HTTP_WAF_MODULE_CTX) as _,
...
extern "C" fn ngx_http_waf_commands_set_enable(
    cf: *mut ngx_conf_t,
    ...
    unsafe {
        ...
        let val = match args[1].to_str() {
            Ok(s) => s,
            Err(error) => {
                ngx_conf_log_error!(NGX_LOG_EMERG, cf, "`waf` argument is not utf-8 encoded",);
