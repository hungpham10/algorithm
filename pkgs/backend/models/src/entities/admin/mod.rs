...
mod tests {
    ...
    async fn test_write_and_read_rdbms_table() {
        ...
        admin
            .setup_database_connection(tenant_id, service.to_string(), db_url.clone())
            .await
            .expect("setup_database_connection should succeed");

        create_real_table(db).await;

        // Create table schema via admin
        let columns = vec![
            ColumnDescription {
                name: Some("id".into()),
                kind: Some(ColumnType::Int64),
        ...
        let rows = admin
            .write_to_table_by_id(tenant_id, table_id, Some(body))
            .await
        ...
        // Read data back
        let data = admin
            .read_from_table_by_id(tenant_id, table_id, 0, 10)
            .await
        ...
        let data_empty = admin
            .read_from_table_by_id(tenant_id, table_id, 9999, 5)
            .await
