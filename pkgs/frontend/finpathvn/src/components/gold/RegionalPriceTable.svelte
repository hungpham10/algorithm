...
<section id="gold-by-province" class="max-w-[1200px] w-full mx-auto px-4 py-12">
  <div class="w-full bg-white border border-slate-200/80 shadow-sm rounded-2xl overflow-hidden">
    <div class="px-5 py-4 bg-slate-50/70 flex flex-row justify-between items-start border-b border-slate-100">
      <div>
        <h2 class="text-3xl font-bold text-slate-800 tracking-tight mt-1">Giá vàng theo vùng miền</h2>
      ...
      <table class="w-full border-collapse">
        <thead>
          <tr class="bg-slate-50/40 text-xs font-semibold border-b border-slate-100">
            <th class="px-6 py-3 text-left font-medium tracking-wide w-[45%]">THƯƠNG HIỆU</th>
            <th class="px-6 py-3 text-right font-medium tracking-wide w-[27.5%]">MUA VÀO</th>
            <th class="px-6 py-3 text-right font-medium tracking-wide w-[27.5%]">BÁN RA</th>
          </tr>
        </thead>
          ...
          {:else if items.length > 0}
            {#each items as item}
              <tr class="hover:bg-slate-50/60 transition-colors group">
                ...
                <td class="px-6 py-4 text-right tabular-nums align-middle">
                  <PriceCell
                    enabled={false}
