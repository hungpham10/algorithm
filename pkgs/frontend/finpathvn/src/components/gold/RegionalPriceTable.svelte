        ...
        try {
            ...
            if (data.query && data.query.length > 0) {
                ...
                const matchedRegion = regions.find(
                    (r) => removeVietnameseTones(r).toLowerCase() === cleanRaw,
                );
                ...
                if (matchedRegion) {
                    ...
                    console.warn(
                        "Không tìm thấy tỉnh khớp trong danh sách:",
                        rawRegion,
                    );
                }
            } else {
...
<section id="gold-by-province" class="max-w-[1200px] w-full mx-auto px-4 py-12">
    <div
        class="w-full bg-white border border-slate-200/80 shadow-sm rounded-2xl overflow-hidden"
    >
        <div
            class="px-4 py-4 sm:px-5 bg-slate-50/70 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100"
        >
            <div>
                <h2
                    class="text-xl sm:text-2xl md:text-3xl font-bold text-slate-800 tracking-tight"
                >
                    Giá vàng theo vùng miền
                </h2>
                <p
                    class="text-[11px] sm:text-sm text-gray-500 mt-0.5 whitespace-nowrap"
                >
                    Đơn vị: triệu đồng/chỉ • Cập nhật <span
                        class="current-date font-medium text-gray-700"
                    ></span>
                </p>
            </div>
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto justify-end">
                ...
                <div class="relative min-w-[130px]">
                    ...
                    <div
                        class="absolute inset-y-0 right-2.5 flex items-center pointer-events-none text-slate-400"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            class="h-3.5 w-3.5"
        ...
        <div class="relative overflow-x-auto">
            <table class="w-full border-collapse">
                <thead>
                    <tr
                        class="bg-slate-50/40 text-xs font-semibold border-b border-slate-100"
                    >
                        <th
                            class="px-4 md:px-6 py-3 text-left font-medium tracking-wide w-[40%] md:w-1/3"
                            >THƯƠNG HIỆU</th
                        >
                        <th
                            class="px-4 md:px-6 py-3 text-right font-medium tracking-wide w-[30%] md:w-1/3"
                            >MUA VÀO</th
                        >
                        <th
                            class="px-4 md:px-6 py-3 text-right font-medium tracking-wide w-[30%] md:w-1/3"
                ...
                <tbody class="divide-y divide-slate-100">
                    {#if loading}
                        {#each Array(5) as _}
                            <tr class="animate-pulse">
                                <td class="px-4 md:px-6 py-4.5"
                                    ><div
                                        class="h-4 bg-slate-100 rounded-md w-2/3"
                                    ></div></td
                                >
                                <td class="px-4 md:px-6 py-4.5 flex justify-end"
                                    ><div
                                        class="h-4 bg-slate-100 rounded-md w-16"
                                    ></div></td
                                >
                                <td class="px-4 md:px-6 py-4.5"
                                    ><div
                                        class="h-4 bg-slate-100 rounded-md w-16 ml-auto"
                                    ></div></td
                                >
                            </tr>
                        {/each}
                    {:else if items.length > 0}
                        {#each items as item}
                            ...
                            >
                                <td
                                    class="p-2 md:p-4 pl-3 md:pl-6 align-middle w-[40%] md:w-1/3"
                                >
                                    <div class="flex flex-col min-w-0">
                                        ...
                                        >
                                            <span
                                                class="text-[14px] md:text-[17px] leading-tight break-words font-semibold text-gray-800 group-hover:text-amber-600 transition-colors"
                                            >
                                                {item.product}
                                            </span>
                                            ...
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                class="h-3.5 w-3.5 mt-0.5 opacity-0 group-hover:opacity-100 text-amber-500 transition-all transform translate-x-[-4px] group-hover:translate-x-0 shrink-0"
                                                fill="none"
                                                viewBox="0 0 24 24"
                                                stroke="currentColor"
                                            >
                                        ...
                                        <span
                                            class="text-[10px] md:text-xs text-gray-400 mt-0.5 whitespace-nowrap"
                                        >
                                            Triệu đồng/chỉ
                                        </span>
                                    </div>
                                ...
                                <td
                                    class="p-3 md:p-4 text-right tabular-nums align-middle text-[16px] md:text-[17px] font-semibold"
                                >
                                    <PriceCell
                                        enabled={false}
                                ...
                                <td
                                    class="p-3 md:p-4 text-right tabular-nums align-middle text-[16px] md:text-[17px] font-bold text-sky-600"
                                >
                                    <PriceCell
                                        enabled={false}
                            ...
                            <td colspan="3" class="px-6 py-16 text-center">
                                ...
                                >
                                    ...
                                    <p class="text-xs italic">
                                        Không tìm thấy dữ liệu giá vàng tại khu
                                        vực {selectedRegion}
                                    </p>
                                </div>
                            </td>
        ...
        {#if items.length > 0 || currentPage > 1}
            <div
                class="px-6 py-3.5 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between"
            >
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <div
                        class="flex items-center justify-center px-3 h-7 border border-slate-200 rounded-lg bg-white text-xs font-semibold text-slate-700 shadow-sm min-w-[2rem]"
                    >
                        {currentPage}
                    </div>
