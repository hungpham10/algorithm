...
BEGIN
    ...
    -- 1️⃣ Tháng tiếp theo: thêm partition mới
    SET next_month = DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 MONTH), '%Y%m');
    SET @sql_add = CONCAT('ALTER TABLE wms_picking_route_events ADD PARTITION (PARTITION p', next_month,
                          ' VALUES LESS THAN (', next_month + 1, '))');
    PREPARE stmt_add FROM @sql_add;
    EXECUTE stmt_add;
    DEALLOCATE PREPARE stmt_add;
    ...
    -- 2️⃣ Tháng quá hạn (>6 tháng): drop partition cũ
    SET old_month = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 6 MONTH), '%Y%m');
    SET @sql_drop = CONCAT('ALTER TABLE wms_picking_route_events DROP PARTITION IF EXISTS p', old_month);
    PREPARE stmt_drop FROM @sql_drop;
    EXECUTE stmt_drop;
    DEALLOCATE PREPARE stmt_drop;
END$$
...
BEGIN
    ...
    -- 1️⃣ Tháng tiếp theo: thêm partition mới
    SET next_month = DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 MONTH), '%Y%m');
    SET @sql_add = CONCAT('ALTER TABLE wms_picking_plan_events ADD PARTITION (PARTITION p', next_month,
                          ' VALUES LESS THAN (', next_month + 1, '))');
    PREPARE stmt_add FROM @sql_add;
    EXECUTE stmt_add;
    DEALLOCATE PREPARE stmt_add;
    ...
    -- 2️⃣ Tháng quá hạn (>6 tháng): drop partition cũ
    SET old_month = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 6 MONTH), '%Y%m');
    SET @sql_drop = CONCAT('ALTER TABLE wms_picking_plan_events DROP PARTITION IF EXISTS p', old_month);
    PREPARE stmt_drop FROM @sql_drop;
    EXECUTE stmt_drop;
    DEALLOCATE PREPARE stmt_drop;
END$$
